...
function processRoundUp(userID, roundUpData) {
    ...
    return plaid.getTransactions(roundUpData.plaid_access_token, startDate, endDate).then(transactions => {
        ...
        return getAPIClient().then(dwolla => {
            return getHoldingID().then(holdingID => {
                const requestBody = {
                    _links: {
                        source: {
                ...
                return dwolla.post('transfers', requestBody).then(() => {
                    ...
                    mailer
                        .sendTemplateToUser(userID, 'Transfer failed', '196a1c48-5617-4b25-a7bb-8af3863b5fcc', {}, ' ', message)
