        ...
        return getCustomerHoldingID(customerID).then(holdingID => {
            ...
            return client.get(`${config.dwolla.url}/funding-sources/${holdingID}/balance`).then(res => {
                return getUserID(customerID).then(userID => {
                    ...
                    const message = `Oh no! A transfer for ${amount} \
                    has failed on ${Date()
                        .toISOstring()
                        .replace(/T/, ' ')
                    ...
                    mailer
                        .sendTemplateToUser(userID, 'Transfer failed', '63fc288b-b692-4d2f-a49a-2e8e7ae08263', bodyDict, 'transfer failed', 'transfer failed')
