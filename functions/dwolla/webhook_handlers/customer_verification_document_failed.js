 ...
 * @returns {Promise}
 */
function customerVerificationDocumentFailedWebhook(body) {
    const custUrl = body._links.customer.href;
    const customerID = custUrl.substr(custUrl.lastIndexOf('/') + 1);
    const documentID = body.resourceId;
    const updates = {};

    updates[`dwolla/customers^documents/${customerID}/${documentID}/status`] = 'failed';
    utils.getUserID(customerID).then(userID => {
        ...
        mailer
            .sendTemplateToUser(userID, 'Document Upload Failed!', '196a1c48-5617-4b25-a7bb-8af3863b5fcc', bodyDict, ' ', message)
            .catch(err => console.error(err));
    });
    return ref.update(updates);
}

module.exports = customerVerificationDocumentFailedWebhook;
